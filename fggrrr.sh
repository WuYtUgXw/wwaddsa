...
stop_port_conflicts() {
    ...
    if systemctl is-active --quiet nginx && command -v netstat &> /dev/null && ! netstat -tulpn | grep -q "nginx.*:80"; then
        log_info "停止Nginx服务..."
...
start_services() {
    ...
    # 确保Docker服务已启动
    log_info "检查并启动Docker服务..."
    if ! systemctl is-active --quiet docker; then
        log_info "启动Docker服务..."
        if ! systemctl start docker; then
            log_error "Docker服务启动失败，请检查Docker安装"
            return 1
        fi
    fi
    ...
    if ! docker ps &> /dev/null; then
        log_error "无法连接到Docker服务，请检查Docker配置"
        return 1
    fi
    
    # 启动服务
    log_info "启动RelayX服务容器..."
